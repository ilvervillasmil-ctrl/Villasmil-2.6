...
def test_respiro_ultra_precision():
    ...
    apply, gain = should_apply(
        current_R=0.99,
        effort_soft={"L1": 0.01},
        effort_hard={"L1": 0.011},
        cost_threshold=100.0
    )
    assert isinstance(apply, bool)
    assert isinstance(gain, (float, str))
    
    # Test 2: should_apply - validar retorno de tupla
    ...
    assert isinstance(apply2, bool)
    assert isinstance(gain2, (float, str))
...
def test_core_theta_todas_branches():
    ...
    assert 0.0 < theta_unknown <= 1.0


def test_core_saturacion_universal():
    """Verifica que OMEGA_U se aplica correctamente"""
    suma1 = core.suma_omega(0.5, 0.5)
    assert suma1 == core.OMEGA_U  # 0.995
    
    suma2 = core.suma_omega(0.6, 0.6)
    assert suma2 == core.OMEGA_U
    
    mc_alto = core.indice_mc(100, 0)
    assert mc_alto == core.C_MAX  # 0.963
