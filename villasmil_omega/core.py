...
guardián_paz = Invariancia(epsilon=1e-3, ventana=5)
...
def compute_theta(cluster: List[Any]) -> float:
    ...
    Aquí está tu lógica original de comparación por strings.
    """
    if not cluster:
        return 0.0
    ...
    # LÓGICA DE CONFLICTO ORIGINAL (Model A vs B)
...
# --- INTEGRACIÓN L4 E INVARIANCIA ---

def procesar_flujo_omega(data: List[Any], directiva: Dict[str, Any]) -> Dict[str, Any]:
    """
    Aquí unimos tu procesar_flujo con la Invariancia.
    """
    # 1. Invariancia (La "mala varianza" o calma)
    num_data = [float(x) for x in data if isinstance(x, (int, float))]
    if guardián_paz.es_invariante(num_data):
        return {"status": "basal", "path": "safety_lock", "invariante": True}

    # 2. Tu lógica de evolución original
